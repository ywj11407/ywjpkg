...
func NewSVN(svnurl string, opts *Options) (*SVN, error) {
	...
	//fmt.Println(workDir, "------", su, "----------", targetBase, "---------", localRepo)
...
// Update file
func (s *SVN) Update(path string, opts map[string]interface{}) error {
	if !s.localRepo {
		return ErrRepoTypeLocal
	}
	_, err := s.execCMD(cmdUpdate, path)
	return err
}
...
func (s *SVN) Diff(localPath string) error {
	if !s.localRepo {
		return ErrRepoTypeLocal
	}
	_, err := s.execCMD(cmdDiff, localPath)
	return err
}
